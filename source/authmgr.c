...
int _sceSblAuthMgrSmLoadSelfBlock(
        ...
        int block_idx)
{
    uint64_t out_offset = block_idx * SELF_SEGMENT_BLOCK_SIZE(segment);
    if (out_offset > segment->uncompressed_size) {
        return -22;
    }

    uint64_t out_size = SELF_SEGMENT_BLOCK_SIZE(segment);
    if (out_size > segment->uncompressed_size - out_offset) {
        out_size = segment->uncompressed_size - out_offset;
    }

    uint64_t size_one;
    uint64_t size_two;
    int is_compressed = block_segment->extents[block_idx]->len != 0 && block_segment->extents[block_idx]->len != out_size;
    if (is_compressed) {
        size_one = block_segment->extents[block_idx]->len & ~0xF;
        size_two = size_one - (block_segment->extents[block_idx]->len & 0xF);
    } else {
        size_one = size_two = out_size;
    }

    struct sbl_msg_header msg = {0};
    struct sbl_authmgr_load_block load = {0};
    msg.cmd                 = 6;
    msg.query_len           = 0x80;
    msg.recv_len            = 0x80;
    msg.message_id          = 0;
    ...
    load.service_id         = service_id;
    load.is_compressed      = is_compressed;
    load.is_plain_elf       = 0;
    load.res                = -1;

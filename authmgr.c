...
int _sceSblAuthMgrSmFinalize(int sock, int authmgr_handle, int context_id)
{
    ...
    msg.cmd       = 6; msg.query_len = 0x80; msg.recv_len = 0x80;
    msg.message_id = 0; msg.to_ret = authmgr_handle;
...
int _sceSblAuthMgrVerifyHeader(int sock, int authmgr_handle,
                               uint64_t header_pa, uint64_t header_len)
{
    int err;
    struct sbl_msg_header           msg = {};
...
int _sceSblAuthMgrSmLoadSelfSegment(int sock, int authmgr_handle,
                                    int service_id,
...
int _sceSblAuthMgrSmLoadSelfBlock(
    int sock, int authmgr_handle, int service_id,
    uint64_t in_pa, uint64_t out_pa,
    struct sce_self_segment_header *segment, int segment_idx,
    struct self_block_segment *block_segment, int block_idx)
{
    struct sbl_msg_header        msg = {};
    struct sbl_authmgr_load_block load = {};
    uint64_t size_one, size_two;
    int res;
    ...
    memcpy(&load.ext_info, block_segment->extents[block_idx],
           sizeof(struct sce_self_block_info));
    ...
    for (int attempt = 0; attempt < 5; ++attempt) {
        res = sceSblServiceRequest(sock, &msg, (char *)&load, (char *)&load);
        if (res == 0 && load.res == 0) {
            if (attempt > 0) {
                printf("  [+] block %d/%lu recovered on attempt %d\n",
                       block_idx + 1, block_segment->block_count, attempt + 1);
            }
            return 0;  // Success
        }

        int err_code = res ? res : load.res;
        printf("  [!] block %d/%lu failed (attempt %d/5, err=%d)\n",
               block_idx + 1, block_segment->block_count, attempt + 1, err_code);

        int delay_ms = 100 << attempt;  // 100, 200, 400, 800, 1600
        if (delay_ms > 1600) delay_ms = 1600;
        usleep(delay_ms * 1000);
    }

    printf("  [!] block %d/%lu failed after 5 attempts\n", block_idx + 1, block_segment->block_count);
    return res ? res : load.res;
}